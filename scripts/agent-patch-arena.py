...
def classify_change(paths: list[str]) -> str:
    ...
    governance = [p for p in paths if matches(p, GOVERNANCE_PATHS)]
    security = [p for p in paths if matches(p, SECURITY_PATHS)]
    performance = [p for p in paths if matches(p, PERFORMANCE_PATHS)]
    known = set(governance + security + performance)
    unknown = [p for p in paths if p not in known]

    # Governance is pure only when every changed path is explicitly governed.
    if governance:
        return "governance" if len(governance) == len(paths) else "mixed"
...
def policy_sha256() -> str:
    canonical = json.dumps(
        policy_document(), sort_keys=True, separators=(",", ":")
    ).encode()
    ...
    with tempfile.TemporaryDirectory(prefix="vortex-arena-") as temp:
        ...
        try:
            ...
            if base_failures or head_failures:
                ...
                result["verdict"] = "REJECT"
                result["reason"] = "mixed change requires explicit review"
            else:
                base_metrics, base_samples = benchmark(base)
                head_metrics, head_samples = benchmark(head)

                throughput_gain = (
                    head_metrics["throughput_ops_s"]
                    / base_metrics["throughput_ops_s"]
                    - 1
                )
                latency_change = (
                    head_metrics["latency_us"] / base_metrics["latency_us"] - 1
                )
                memory_change = (
                    head_metrics["rss_mb"] / base_metrics["rss_mb"] - 1
                )
                base_cv = coefficient_of_variation(
                    [s["throughput_ops_s"] for s in base_samples]
                )
                head_cv = coefficient_of_variation(
                    [s["throughput_ops_s"] for s in head_samples]
                )
                ...
                if change_class == "performance":
                    ...
                    reason = (
                        "candidate meets performance improvement and stability policy"
                        if performance_ok
                        else "candidate does not demonstrate sufficient measured gain"
                    )
                elif change_class in {"security", "correctness"}:
                    verdict = "PASS_NO_REGRESSION" if no_regression_ok else "REJECT"
                    reason = (
                        "candidate meets bounded no-regression policy"
                        if no_regression_ok
                        else "candidate exceeds bounded regression policy"
                    )
                else:
                    verdict = "PASS_GOVERNANCE" if no_regression_ok else "REJECT"
                    reason = (
                        "governance change passes bounded no-regression policy; "
                        "protected-branch review remains mandatory"
                        if no_regression_ok
                        else "governance change exceeds bounded regression policy"
                    )
